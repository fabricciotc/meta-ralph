...
```bash
export AGENTICFLOW_BACKENDS="kimi claude cursor codex copilot openai_api"
export AGENTICFLOW_BACKEND=auto
```

Use one backend explicitly:

```bash
AGENTICFLOW_BACKEND=claude meta-ralph run
AGENTICFLOW_BACKEND=codex meta-ralph run
AGENTICFLOW_BACKEND=cursor meta-ralph run
AGENTICFLOW_BACKEND=copilot meta-ralph run
```
...
The source of truth is the application state directory (`state/board.json`).
...
Application data lives in the OS-native app data directory (e.g. `~/Library/Application Support/AgenticFlow` on macOS, `%LOCALAPPDATA%/AgenticFlow` on Windows, `~/.local/share/AgenticFlow` on Linux):

```text
<app-data-dir>/
├── config.json           # Backend preference, projects root, maxWorkers
├── logs/                 # Engine logs
├── state/
...
6. **Cap parallelism**. Default max workers is 10, but increase or reduce it for rate limits, small batches, or hosts without background agents.
...
- Work on a dedicated branch, usually `agenticflow/*`.
