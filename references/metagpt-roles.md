...
> **Paths:** `<AGENTICFLOW_DATA_DIR>` refers to the AgenticFlow application data directory. If the environment variable is unset, the default is OS-native: `~/Library/Application Support/AgenticFlow` on macOS, `%LOCALAPPDATA%/AgenticFlow` on Windows, or `~/.local/share/AgenticFlow` on Linux.
...
- A research note at `<AGENTICFLOW_DATA_DIR>/state/pm-research/<agent_id>.md`.

### SOP

1. Read `prd.json` completely.
2. Identify the assigned board ticket in `<AGENTICFLOW_DATA_DIR>/state/board.json`.
...
- `prd.json` with user stories.
- PM Research notes from `<AGENTICFLOW_DATA_DIR>/state/pm-research/*.md`.
...
1. Read `prd.json` completely.
2. Read `<AGENTICFLOW_DATA_DIR>/state/board.json` to understand existing tickets.
...
- `architecture.md`.
- `prd-expanded.json`.
- An isolated worktree at `<AGENTICFLOW_DATA_DIR>/state/worktrees/<task_id>/`.
...
2. Identify the associated ticket in `<AGENTICFLOW_DATA_DIR>/state/board.json`; the Orchestrator should have moved it to **In Progress**.
